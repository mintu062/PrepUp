...
public class RestCreateExamController {
	...
	public Status createClass(@RequestBody CreateExamDetails createExamDetails) {
		Boolean status=examDetailsService.createExam(createExamDetails);
		Status s = new Status();
		if(status==true) {
		s.setStatus_code(200);
		s.setMessage("Your exam is created");
		}
		else {
